...
from data import VOCroot, COCOroot, VOC_300, VOC_512, COCO_300, COCO_512, COCO_mobile_300, AnnotationTransform, COCODetection, OCRDetection, VOCDetection, detection_collate, BaseTransform, preproc
...
parser.add_argument(
    '--basenet', default='./logs/vgg16_reducedfc.pth', help='pretrained base model')
...
args.ngpu = gpus
args.max = max_iters
# args.save_folder = join(data_dir, 'logs')
...
num_classes = len(load_classes(join(data_dir,'ocr.names'))) + 1
...
def train():
    ...
    if args.dataset == 'VOC':
        ...
        #     img_dim, rgb_means, p))
        dataset = OCRDetection(data_dir, train_sets, preproc(
            img_dim, rgb_means, p))
    else:
